...
# ----------------- IntentDetector (OpenAI-first) ----------------- #
class IntentDetector:
    """
    OpenAI-first intent detector.
    - Uses generative model (OpenAI) to map free text -> structured JSON intent + entities.
    - Falls back to a light rule+NER extractor when the LLM fails or has low confidence.
    """
    ...
    def detect_intent(
        ...
        # Call OpenAI to get structured intent
        openai_response = self._call_openai_for_intent(message, inferred_channel, relevant_items, previous_context)

        parsed = _safe_parse_json(openai_response)

        if parsed:
            # Normalize and return
            normalized = self._normalize_openai_output(parsed, inferred_channel)
        ...
        # --- fallback to lightweight rules + NER (if openai fails or low confidence) ---
        fallback = self._fallback_rules_and_ner(message, inferred_channel, relevant_items)
        fallback["source"] = "fallback"
        fallback["entities"]["input_text"] = message  # ensure query_text is always present
        return fallback
    ...
    "entities": {{
        ...
        try:
            ...
            try:
                ...
                logger.warning("Failed to parse OpenAI JSON; returning raw.")
                return raw_text

        except Exception as e:
            logger.exception("OpenAI chat.completions.create failed: %s", e)
            return ""
    ...
    # ----------------- Normalization ----------------- #
    def _normalize_openai_output(self, parsed: Dict[str, Any], channel_hint: str) -> Dict[str, Any]:
        """
        Ensure returned JSON contains required fields and sensible defaults.
        Also normalize timeframe -> parsed_time where possible.
        """
    ...
    def _fallback_rules_and_ner(self, message: str, channel: str, relevant_items: List[Any]) -> Dict[str, Any]:
        """
        Lightweight fallback when OpenAI fails: keyword scoring + spaCy NER + date parsing.
        """
        text = message.lower()
        best_intent = "unknown"
        best_score = 0.0
