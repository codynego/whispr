...
class TodoService:
    ...
    def update_todo(
        self, todo_id: int, *, task: Optional[str] = None, done: Optional[bool] = None
    ) -> Optional[Todo]:
        """
    ...
    def mark_done(self, todo_id: int) -> Optional[Todo]:
        """Mark a todo as completed (done=True)."""
        return self.update_todo(todo_id, done=True)

    def mark_undone(self, todo_id: int) -> Optional[Todo]:
        """Unmark a todo (done=False)."""
        return self.update_todo(todo_id, done=False)
